...
class BaseDB(object):
    def __init__(self, args):
        if "wb" in args.data:
            ...
            #self.max_srclen, self.max_tgtlen = 150, 55
            ...
            #self.max_srclen, self.max_tgtlen = 120, 50
            self.max_srclen, self.max_tgtlen = 60, 20
            self.dataset = "giga"
            self.min_src_count = 100
        print("btw dataset:", self.dataset)
...
class ValDB(BaseDB):
    def __init__(self, args):
        ...
        # if args.ne_thresh:
        #     thresh, nclosene = args.ne_thresh
        ...
        for i in range(len(neidxs)):
            ...
            # self.val_neidxs.append(neidxs[i])
            # neidx_set = set(neidxs[i])
            # self.val_neidxs[-1].extend(protes - neidx_set)
            # self.val_ne_dists.append(nedists[i])
