...
def se_search(batchidxs, nelist, neoffs, K, model, db, device, max_moves=25,
    ...
    srcs, ufeats, neighbs, canvases, relidxs, lengths, _ = sutils.init_search(
        batchidxs, nelist, db)
    ...
    encsrc, enccanv, _ = model.src_encode(
        srcs, ufeats, lengths, canvases, relidxs, db.pad_idx)
    ...
    while len(remaining) > 0:
        ...
        startlps = model.actmodel.get_start_lps( # bsz x canvlen*(nelen*nne+V+srclen)
            enccanv, encne, nemask, encsrc, srcs, model.lut, pad_idx=db.pad_idx)
        ...
        # but that's probably too big. Instead we'll take each topk or something
        ...
        canvases, relidxs, lengths, ufeats = sutils.get_updated_canvs(hyps, db, device)
        ...
        encsrc, enccanv, _ = model.src_encode(srcs, ufeats, lengths, canvases, relidxs, db.pad_idx)
...
def lr_search(batchidxs, nelist, neoffs, K, model, db, device, max_moves=25,
              min_len=0, max_canvlen=200, len_avg=True, leftright=False, only_copy=False):
    model.eval()
    bsz = len(batchidxs)
    fin_hyps = [[] for _ in range(bsz)]
    ...
    while len(remaining) > 0:
        ...
        startlps = model.actmodel.get_start_lps( # bsz x nelen*nne+V+srclen
            enccanv, encne, nemask, encsrc, srcs, model.lut, pad_idx=db.pad_idx)
        assert startlps.size(1) == canvlen*colpercanv
        # do it again
        endlps = model.actmodel.get_end_lps2( # bsz x nelen*nne+V+srclen
            enccanv, encne, nemask, encsrc, srcs, model.lut, pad_idx=db.pad_idx)

        # somehow combine shit
        comblps = combine_lps(startlps, endlps)
        comblps.add_(scores.view(-1, 1))
        nnetoks = neighbs.nelement()
        startlps.view( # disallow unk
            startlps.size(0), canvlen, -1)[:, :, nnetoks + db.d.w2i["<unk>"]].fill_(-float("inf"))
        if only_copy: # can't invent any words
        ...
        for b in range(bsz):
            nuhyps_b = []
            for k in range(argmaxes.size(1)):
                if torch.isneginf(maxes[b, k]): # beam too big and only illegal options left
                    break
                parent = argmaxes[b, k].item() // endlps.size(1)
                endidx = argmaxes[b, k].item() % endlps.size(1)
                predr, predk = endidx2pred(endidx, remembs.size(0))
