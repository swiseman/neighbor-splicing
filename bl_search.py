...
def search(batchidxs, nelist, neoffs, K, model, db, device, max_moves=50, min_len=0,
           len_avg=True, no_agg=False, only_copy=False):
    model.eval()
    bsz = len(batchidxs)
    fin_hyps = [[] for _ in range(bsz)]
    if args.get_trace:
        hyps = [[Hyp([("<bos>", "(s)")], 0)] for _ in range(bsz)]
    else:
    ...
    with open(args.out_fi, "w+") as f:
        for ii in range(start, end, args.bsz):
            ...
            batch_fin_hyps = search(batchidxs, nelist, neoffs, K, model, db, device,
                                    ...
                                    len_avg=(not args.no_len_avg), no_agg=args.no_agg,
            ...
            for b in range(len(batchidxs)):
                ...
                if db.tokenizer is None:
                    if args.get_trace:
                        ahyp, atrace = zip(*fin_hyps[argmax.item()].hyp)
                    else:
                        ahyp = fin_hyps[argmax.item()].hyp
...
parser.add_argument('-no_agg', action='store_true', help='')

if __name__ == "__main__":
    args = parser.parse_args()
    args.arbl = True
    args.leftright = False
    args.min_nes = 0 if args.no_ne else 1
    print(args)
